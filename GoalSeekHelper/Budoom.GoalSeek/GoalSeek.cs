...
namespace Budoom
{
    public class GoalSeek
    {
        ...
        public GoalSeekResult TrySeek(decimal targetValue = 0, decimal guess = 0, int maxIterations=DefaultMaxIterations, bool resultRoundOff=DefaultResultRoundOff)
        {
            return TrySeek(func, DefaultAccuracyLevel, targetValue, guess, maxIterations, resultRoundOff);
        }
...
*/
        public static GoalSeekResult TrySeek(Func<decimal, decimal> func, decimal accuracyLevel = DefaultAccuracyLevel, decimal targetValue = 0, decimal guess = 0, int maxIterations = DefaultMaxIterations, bool resultRoundOff = DefaultResultRoundOff)
        {
            ...
            if (Math.Abs(result1) <= accuracyLevel && iterations <= maxIterations)
                isGoalReached=true;

            if (resultRoundOff)
                guess = Math.Round(guess, (accuracyLevel.ToString().Length - (accuracyLevel.ToString().IndexOf('.') + 1)));

            return new GoalSeekResult(targetValue: targetValue, accucracyLevel: accuracyLevel, iterations: iterations, isGoalReached: isGoalReached, closestValue: guess);
        }
    }
}
